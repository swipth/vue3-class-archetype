module.exports = {
  extends: [
    'stylelint-config-standard','stylelint-order' // 使用标准的Stylelint配置
  ],
  customSyntax: "postcss-html",
  rules: {
    // 在这里添加您自定义的规则
    "unit-case": "lower",//指定单位的大小写（可自动修复）。
    "font-weight-notation": "numeric",//要求 font-weight 使用数字或命名（如果可能）值。此外，当需要命名值时，要求名称有
    "selector-no-vendor-prefix": null,//禁止选择器的浏览器引擎前缀。
    ...
    "value-no-vendor-prefix": null,//禁止值的浏览器引擎前缀。
    "length-zero-no-unit": true,  // 禁止零长度的单位（可自动修复）
    "block-no-empty": true,  // 禁止空对象
    "unit-no-unknown": true,  // 禁止未知单位
    "shorthand-property-no-redundant-values": true, //禁止简写属性的冗余值（可自动修复）
    "number-leading-zero": "never", // 要求或禁止小于 1 的小数有一个前导零（可自动修复）。
    "number-no-trailing-zeros": true,//禁止数量的尾随零（可自动修复）。
    "string-quotes": "double",//指定字符串使用单引号或双引号（可自动修复）。
    "declaration-block-no-duplicate-properties": true, // 禁止声明快重复属性
    "no-descending-specificity": true, // 禁止在具有较高优先级的选择器后出现被其覆盖的较低优先级的选择器。
    "selector-max-id": 1, // 限制一个选择器中 ID 选择器的数量
    "max-nesting-depth": 5, // 样式层级深度
    "function-comma-newline-after": "always", //要求在函数的逗号之后必须有换行符或不能有空白符（可自动修复）。
    "function-comma-newline-before": "always", //要求在函数的逗号之前必须有换行符或不能有空白符（可自动修复）。
    "function-comma-space-after": "always", //要求在函数的逗号之后必须有一个空格或不能有空白符（可自动修复）。
    "function-comma-space-before": "always", //要求在函数的逗号之前必须有一个空格或不能有空白符（可自动修复）。
    "function-max-empty-lines": 1,//限制函数中相邻空行的数量（可自动修复）
    "function-name-case":  [
      "lower",
      {
        "ignoreFunctions": ["rgba","rgb"]
      }
    ], //指定函数名的大小写（可自动修复）。
    "function-parentheses-newline-inside": "always",//要求在函数的括号内侧必须有换行符或不能有空白符（可自动修复）。
    "function-parentheses-space-inside": "always",//要求在函数的括号内侧必须有一个空格或不能有空白符（可自动修复）。
